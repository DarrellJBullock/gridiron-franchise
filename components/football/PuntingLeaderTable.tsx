...
import type { PuntingLeaderRow } from "@/lib/stats/leaders";

export function PuntingLeaderTable({ rows }: { rows: PuntingLeaderRow[] }) {
  return (
    <Card className="p-5">
      <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-text-muted">Punting Leaders</p>
      {rows.length === 0 ? (
          ...
          <Table>
            <Thead>
              <Tr>
                ...
                <Th>Pos</Th>
                <Th>Team</Th>
                <Th className="text-right">Punts</Th>
            ...
            <Tbody>
              {rows.map((row, i) => (
                <Tr key={row.playerId}>
                  ...
                  <Td className="text-right tabular-nums text-text-muted">{row.punts}</Td>
