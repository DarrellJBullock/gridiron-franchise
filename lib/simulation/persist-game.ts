  ...
  if (result.playerStats.length > 0) {
    await prisma.gamePlayerStats.createMany({
      data: result.playerStats.map((line) => ({
        ...
        passingAttempts: line.passingAttempts ?? 0,
        passingCompletions: line.passingCompletions ?? 0,
